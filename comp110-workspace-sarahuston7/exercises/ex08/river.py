...
class River:
    ...
    def remove_fish(self, amount: int):
        ...
        for elem in range(amount, len(self.fish)):
            new_fish_list.append(self.fish[elem])
    ...
    def bears_eating(self):
        ...
        while g < len(self.bears):
            print(f"this is the len of bears {len(self.bears)}")
            print(f"this is the len of fish {len(self.fish)}")
            if len(self.fish) >= 5:
    ...
    def check_hunger(self):
        ...
        while d < len(self.bears):
            if self.bears[d].hunger_score >= 0:
                print(self.bears[d].hunger_score)
                not_starved_bears.append(self.bears[d])
            d += 1
            print(len(not_starved_bears))
    ...
    def repopulate_fish(self):
        ...
        new_fish_list: list[Fish] = []
        length_fish: int = len(self.fish)
        # if length_fish % 2 != 0:
        #     length_fish -= 1
        length_fish = length_fish // 2
        for indexes in range(0, length_fish * 4):
            baby_fish: Fish = Fish()
            new_fish_list.append(baby_fish)
        self.fish += new_fish_list
        return None
    ...
    def repopulate_bears(self):
        ...
        length_bears: int = len(self.bears)
        if length_bears % 2 != 0:
            length_bears -= 1
        for indexes in range(0, length_bears // 2):
            baby_bear: Bear = Bear()
            new_bear_list.append(baby_bear)
